        ...
        <table>
            <tr>
                ...
                <?php foreach ($experiments['Fermenter'] as $fermenter): ?>
                    <th><?php echo $fermenter['name']; $ferms[] = $fermenter['name']; ?> <a href="" id="selectall-<?php echo $fermenter['name']; ?>">select all</a></th>
                <?php endforeach; ?>
            </tr>
                ...
                <?php foreach ($ferms as $ferm_id): ?>
                    ...
                    <?php if (array_key_exists($ferm_id, $dates)):
                        foreach($dates[$ferm_id] as $time => $tp):
                            ...
                            <span id="<?php echo $tp_id; ?>"><?php echo $time ?> (<?php echo $tp['diff']; ?>)</span><br />
                        <?php endforeach; ?>
                    <?php else: ?>
                        &nbsp;
                    <?php endif; ?>
    ...
    $(document).ready(function() {
        ...
        a.click(function(event) {
            ...
            $("span").each(function(index) {
                if ($(this).attr("id").substring(0, 3 + ferm.length) == "TP-"+ferm) {
